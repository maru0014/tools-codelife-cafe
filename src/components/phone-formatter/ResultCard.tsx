 ...
 */
import { Phone, Smartphone, Globe, Hash } from 'lucide-react';
...
export default function ResultCard({ result }: ResultCardProps) {
  ...
  const typeWithRegion = regionName ? `${typeLabel}（${regionName}）` : typeLabel;

  const formatRows = [
    { label: 'E.164', value: formats.e164, ariaLabel: 'E.164形式をコピー' },
    { label: '国際表記', value: formats.international, ariaLabel: '国際表記をコピー' },
    { label: '国内表記', value: formats.national, ariaLabel: '国内表記をコピー' },
    { label: 'RFC 3966', value: formats.rfc3966, ariaLabel: 'RFC3966形式をコピー' },
  ];
        ...
        {formatRows.map((row) => (
          ...
          >
            <div className="min-w-0 flex-1">
              ...
              <p className="font-mono text-sm font-medium truncate" title={row.value}>
                {row.value}
              </p>
            </div>
            <CopyButton
