  ...
  return (
    <div className="space-y-4">
      ...
      <div className="overflow-x-auto rounded-xl border border-border">
        <table className="w-full text-sm" role="grid" aria-label="変換結果一覧">
          <thead>
              ...
              return (
                <tr
                  ...
                  className={`border-t border-border transition-colors ${
                    !result.valid ? 'bg-red-50/50 dark:bg-red-950/20' : 'hover:bg-muted/20'
                  }`}
                >
                  ...
                  {COLUMNS.filter((c) => c.id !== '#' && c.id !== 'status' && visibleColumns.includes(c.id)).map((col) => (
                  ...
                  {visibleColumns.includes('status') && (
                    <td className="px-3 py-2 text-xs">
                      {result.valid ? (
                        ...
                        <span className="text-red-600 dark:text-red-400 font-medium" title={result.error}>
                          ❌
                        </span>
                      )}
                    </td>
      ...
      {totalPages > 1 && (
        ...
        >
          <span className="text-xs text-muted-foreground">
            ...
            {Math.min(page * PAGE_SIZE, results.length).toLocaleString()}件（全{results.length.toLocaleString()}件）
          </span>
