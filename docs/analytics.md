...
> **blob スロット対応表**: `blob1` = イベント名、`blob2` = ツールslug（`tool` / `from`）、`blob3` = 補助1（`source` / `to` / `lengthBucket`）、`blob4` = 補助2（`setId` / `hasJapanese`）、`blob5` = 匿名セッションID。`index1` = イベント名（インデックスは 1 データポイント 1 つのみ）。
