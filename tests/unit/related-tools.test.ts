...
test('目的別ブロックは実在する公開ツールのみを参照する', () => {
	...
	for (const purpose of purposeCategories) {
		assert.ok(purpose.tools.length > 0, `${purpose.title} にツールがない`);
		for (const tool of purpose.tools) {
			const catalogTool = publicTools.get(tool.id);
			assert.ok(
				catalogTool,
				`${purpose.title} の ${tool.id} が公開カタログに存在しない`,
			);
			assert.equal(
				tool.href,
				catalogTool.href,
				`${tool.id} の href が catalog と一致しない`,
			);
		}
	}
});
