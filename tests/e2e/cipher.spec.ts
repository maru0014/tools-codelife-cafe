...
test.describe('Cipher Tool', () => {
  ...
  test('Page loads correctly and defaults to Caesar cipher', async ({ page }) => {
  ...
  test('Tab switching resets state and changes UI', async ({ page }) => {
    const inputArea = page.getByPlaceholder(/「こんにちは」や「Hello World」を入力.../)
  ...
  test('String reversal transformation', async ({ page }) => {
    ...
    const inputArea = page.getByPlaceholder(/「テキストを反転します」を入力.../)
  ...
  test('Brute force panel functionality', async ({ page }) => {
    ...
    // Open brute force
    await page.getByRole('button', { name: 'Toggle brute force panel' }).click()
    
    // The panel should show 25 items for ASCII
    await expect(page.locator('button').filter({ hasText: 'シフト 1:' }).filter({ hasText: 'bcd' })).toBeVisible()
    await expect(page.locator('button').filter({ hasText: 'シフト 25:' }).filter({ hasText: 'zab' })).toBeVisible()

    // Click a pattern to set shift
    await page.locator('button').filter({ hasText: 'シフト 5:' }).filter({ hasText: 'fgh' }).click()
