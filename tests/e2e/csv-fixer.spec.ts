...
test.describe('CSV文字化け修復ツール', () => {
  ...
  test('Shift_JISファイルが正しく検出・変換されること', async ({ page }) => {
    ...
    await page.locator('input[type="file"]').setInputFiles(path.join(process.cwd(), 'tests', 'e2e', 'fixtures', 'shift_jis.csv'));
    ...
    const outputEncodingSelect = page.getByRole('combobox', { name: '出力エンコーディング' });
  ...
  test('無効なファイルフォーマットが拒否されること', async ({ page }) => {
    await page.locator('input[type="file"]').setInputFiles(path.join(process.cwd(), 'tests', 'e2e', 'fixtures', 'invalid.xlsx'));

    // Check error message
    await expect(page.getByText('CSV、TSV、TXTファイルのみ対応しています。')).toBeVisible();
  });
  ...
  test('即時変換モードが動作すること', async ({ page, context }) => {
    ...
    await expect(page.getByRole('button', { name: 'ダウンロード完了' })).toBeVisible();
  });
  test('EUC-JPファイルが正しく検出され、警告が出ないこと', async ({ page }) => {
    // Choose file
    await page.locator('input[type="file"]').setInputFiles(path.join(process.cwd(), 'tests', 'e2e', 'fixtures', 'euc_jp.csv'));
